...
class PyannoteDiarizer:
    ...
    def diarize(
        ...
        if audio_file.has_file_extension("wav"):
            ...
            # Debugging before extracting audio
            print(f"Extracting audio for diarization: {media.path}")
            assert media.path != "", "Media path is empty!"
            assert os.path.exists(media.path), f"Media path {media.path} does not exist!"
