...
def resize(
    ...
    """
    # Debugging media file initialization
    print(f"Initializing AudioVideoFile with video_file_path: {video_file_path}")
    assert video_file_path != "", "Video file path is empty!"
    assert os.path.exists(video_file_path), f"Video file path {video_file_path} does not exist!"
    print(f"Initializing AudioVideoFile with video_file_path: {video_file_path}")
    media = AudioVideoFile(video_file_path)
    print(f"Media path: {media.path}")
    media.assert_has_audio_stream()
    media.assert_has_video_stream()

    # Debugging diarization step
print(f"Starting diarization for media: {video_file_path}")
print(f"min_segment_duration: {min_segment_duration}, time_precision: {time_precision}")

    logging.debug("DIARIZING VIDEO ({})".format(media.get_filename()))
    print(f"Starting diarization with min_segment_duration: {min_segment_duration}, time_precision: {time_precision}")
    ...
    print(f"Detecting scenes with min_scene_duration: {min_scene_duration}")
    ...
    print(f"Resize operation completed. Crops: {crops}")
